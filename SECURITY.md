...
Security fixes are applied to the latest published Atlas release.
...
Include the Atlas version, Windows version, reproduction steps using synthetic data, expected behavior, and observed behavior. Remove local paths, usernames, tokens, logs containing content, and screenshots containing personal information.

## Current security boundary

- Atlas is a single-user, local-first application.
- `atlasd` binds only to `127.0.0.1` and uses a local authentication token.
...
- Atlas does not silently enable cloud hosting, a paid AI provider, or an external write integration.
...
- Atlas data is protected by the Windows account and local device controls. Atlas does not provide application-level encryption for the SQLite database.
