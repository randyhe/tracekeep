...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($process) {
        ...
        if (-not $actualPath -or -not [string]::Equals($actualPath, $expectedNodePath, [System.StringComparison]::OrdinalIgnoreCase)) {
            throw "Refusing to stop process $processId because it is not this Tracekeep release's bundled Node runtime."
        }
...
& $codexCopy plugin remove tracekeep@tracekeep-release
if ($LASTEXITCODE -ne 0) { Write-Warning "Tracekeep plugin was already absent or could not be removed." }
& $codexCopy plugin marketplace remove tracekeep-release
if ($LASTEXITCODE -ne 0) { Write-Warning "Tracekeep marketplace was already absent or could not be removed." }

Write-Host "Tracekeep was removed from Codex. Data remains at: $(Join-Path $workPath 'data')"
