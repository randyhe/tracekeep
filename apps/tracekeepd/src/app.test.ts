...
let storage: TracekeepStorage;
let app: FastifyInstance;

beforeEach(async () => {
  storage = new TracekeepStorage(":memory:", ".runtime-test/backups");
...
describe("tracekeepd", () => {
  ...
  it("runs the capture-review-today-search lifecycle", async () => {
    const capture = await app.inject({
      ...
      payload: { text: "Finish Tracekeep integration", title: "Finish Tracekeep integration" },
    });
    ...
    const search = await app.inject({ method: "GET", url: "/api/v1/search?q=Tracekeep" });
    expect(search.json().results.length).toBeGreaterThan(0);
    expect(search.json().results[0]).toMatchObject({
      sourceTitle: "Finish Tracekeep integration",
      sourceType: "manual",
    });
  });
  ...
  it("can pause automatic turn capture without disabling manual Tracekeep workflows", async () => {
  ...
  it("protects private APIs with a local bearer or HttpOnly session", async () => {
    const protectedStorage = new TracekeepStorage(":memory:", ".runtime-test/auth-backups");
  ...
  it("redacts every restricted candidate in plural import responses", async () => {
    const canary = "TRACEKEEP_IMPORT_CANARY_4AC82";
    const response = await app.inject({
      method: "POST",
      url: "/api/v1/imports/daily-log",
