...
import { restoreInputSchema } from "@atlas/contracts";
import { AtlasStorage, fingerprint } from "@atlas/storage";
...
const port = Number.parseInt(args.port ?? process.env.ATLAS_PORT ?? "4310", 10);
if (await isPortListening(port)) {
  throw new Error(`atlasd is running on 127.0.0.1:${port}; stop the service before restore`);
}

const lock = acquireLifetimeLock(dataDirectory, "restore");
let storage: AtlasStorage | undefined;
try {
  storage = new AtlasStorage(resolve(dataDirectory, "atlas.sqlite"), resolve(dataDirectory, "backups"));
