...
$audioDirectory = if ($KokoroClipDirectory) { $KokoroClipDirectory } else { Join-Path $outputDirectory "narration-clips" }
...
$starts = @(2000, 10000, 35000, 60000, 85500, 105500, 126000, 151000)
if ($paragraphs.Count -ne $starts.Count) { throw "Expected $($starts.Count) narration paragraphs, found $($paragraphs.Count)." }

if ($KokoroClipDirectory) {
  $clipPaths = 1..$paragraphs.Count | ForEach-Object { Join-Path $audioDirectory ("cue-{0:d2}.wav" -f $_) }
  foreach ($clipPath in $clipPaths) {
    if (-not (Test-Path -LiteralPath $clipPath -PathType Leaf)) { throw "Kokoro narration clip was not found: $clipPath" }
  }
} else {
...
for ($index = 0; $index -lt $clipPaths.Count; $index += 1) {
  $inputIndex = $index + 1
  $delay = $starts[$index]
  $tempo = if ($KokoroClipDirectory -and $index -eq 4) { "atempo=1.11," } elseif ($KokoroClipDirectory -and $index -eq 7) { "atempo=1.14," } else { "" }
  $filters += "[$($inputIndex):a]$($tempo)adelay=$delay|$delay[a$index]"
}
