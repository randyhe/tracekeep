...
**Atlas 是一个以 Codex 对话为入口的本地第二大脑。** 安装并启用后，每当一次有价值的对话回合结束，它会自动把真正值得保留的内容沉淀成带来源的学习笔记、行动和决定。研究过的论文、发给 Codex 的文档、一个有用的网址、做到一半的生活安排，或一闪而过但值得继续的想法，都不会因为下一次打断而消失。

**它会替你记住：刚才做到哪里？下一步做什么？为什么值得继续？**

![Atlas 自动沉淀日常经验，并在之后的任务中带来源找回](docs/assets/atlas-chat-first-uat.png)

*基于产品负责人真实 UAT 制作的隐私安全复现：一次普通对话被自动沉淀，随后在另一个 Codex 任务中带来源找回。Atlas 当前正式交付的是本地 Codex 插件；具体宿主界面可能有所变化。*
...
2. **需要时就问**
   ...
   > 搜索 Atlas：我之前发现什么方法可以避免早上忘记带东西？请显示来源。
...
你不需要每次都说“请记住”。一次有价值的 Codex 回合结束后，Atlas 会自动保存学习内容，并把可能影响计划的行动或决定放进 Review。明确记录仍然保留，适合你希望马上确认保存的事情。
...
- **Dashboard：** 浏览器打开 Atlas，并能看到 `Today`、`Learning`、`Review` 和 `Search`。
...
- **让对话自然沉淀：** 有价值的 Codex 回合结束时，Atlas 自动提取值得保留的内容。
- **带着上下文继续：** 询问还有哪些事情没有闭环，或搜索以前的记录和来源。

本地 Stop Hook 只处理 Atlas 安装、信任并启用之后完成的回合，不会扫描全部历史。简短寒暄和疑似密码、Token 的内容会跳过。个人对话中的有用结论、文档、论文和网址会自动成为 Learning Notes；行动和决定仍进入 Review。工作摘要、Restricted 内容和不确定内容不会自动接纳。

Atlas 保存结构化结果、受限长度的摘要、来源标识和回忆所需的证据，不执行导入文字中的命令，也不会自动打开捕获到的网址。服务临时离线时，插件使用本地私有队列稍后重试。你可以在 **Settings** 随时暂停自动沉淀；主动记录、搜索和回忆仍可继续使用。
...
Atlas 计划中的手机体验是 **ChatGPT Direct**，不是用手机浏览器远程操作电脑 Dashboard。用户可以直接在 ChatGPT 手机对话里告诉 Atlas“记住这件事”，也可以随时问“我有哪些做到一半的事情”。

这项能力尚未包含在 v0.3.0。目标架构是 ChatGPT App 通过远程 HTTPS MCP 网关和 OAuth 2.1 完成身份认证，再由用户电脑上的轻量同步程序主动取回待审核记录，交给本地 `atlasd` 写入 SQLite。SQLite 仍是唯一权威数据源；远程网关只是有时限的传输队列，不保存整套 Atlas 数据库，默认也不复制完整对话。
...
产品、隐私、费用和发布决定仍由人做出。Atlas 通过公开提交、测试证据、能力探针和明确的声明边界记录这段协作，不把模型生成内容描述成无人负责的自动产品决策。
