...
> **把聊天里没做完的事，变成不会丢失、随时能继续的下一步。**

你不是没有想法，而是想法散落在聊天、搜索和做到一半的任务里。一个会议、一条消息或一个新问题，就可能让你忘记刚才做到哪里、下一步是什么，以及那个灵感为什么重要。传统待办工具还要求你停下手头工作，打开另一个 App，再重新整理一遍。
...
1. **想到时就说：** `Atlas，请记住：下次继续完善演示视频的开场。`
2. **Atlas 先让你审核：** 它保存为一条待确认记录（Review Candidate），不会擅自执行，也不会把整段聊天都存下来。
3. **需要时直接问：** `Atlas，我有哪些做到一半的事情？` 或 `Atlas，今天最值得继续什么？`

例如，你在聊天里说“以后把这个测试加入 CI”，Atlas 可以把它保留为一个带来源的未完成事项；你说“第一版决定使用 SQLite”，Atlas 可以把它保留为一个可追溯的决定。**你不需要先整理好自己，Atlas 帮你在思考发生的地方抓住下一步。**
...
> **Turn unfinished thoughts in your conversations into the next action you can actually resume.**

You do not run out of ideas. You lose them between chats, searches, meetings, and interrupted work. A new message or urgent task makes it easy to forget where you stopped, what the next step was, and why an idea mattered. Traditional task apps add another interruption: stop working, open another app, and organize everything again.
...
1. **Say it when it happens:** `Atlas, remember this: improve the opening of the demo video next time.`
...
For example, “Add this test to CI later” can become a sourced unfinished action. “We decided to use SQLite for V1” can become a traceable decision. **You do not have to organize yourself before capturing a thought; Atlas catches the next step where the thinking already happens.**
