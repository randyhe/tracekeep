...
Tracekeep should work where a thought already happens: inside a ChatGPT mobile conversation. The user should not need to open a separate task app or remotely operate a desktop Dashboard just to remember or resume something.

Two interactions define the mobile experience:

1. **Say it when it happens**

   > Tracekeep, remember that I still need to compare the pickup times for the three summer camps.

2. **Ask when it is needed**

   > Tracekeep, what unfinished family tasks should I resume today?
...
1. The user explicitly asks Tracekeep to remember an idea, decision, reference, or future action in ChatGPT mobile.
2. The Tracekeep ChatGPT App calls a scoped `capture` tool.
...
5. Local `tracekeepd` creates a `Source`, `Capture`, `Evidence`, and `ReviewCandidate` in one transaction.
...
1. The user asks Tracekeep what remains unfinished or searches for an earlier decision.
2. The ChatGPT App calls a read-scoped tool through the gateway.
3. When the PC is online, the gateway relays the request over an authenticated outbound session established by the PC. Local `tracekeepd` returns only the minimum structured result and bounded source evidence.
...
```mermaid
flowchart LR
    User["ChatGPT mobile user"] --> App["Tracekeep ChatGPT App"]
    ...
    Agent --> Tracekeepd["tracekeepd on 127.0.0.1"]
    Tracekeepd --> SQLite["Local SQLite"]
    Dashboard["Local Web Dashboard"] --> Tracekeepd
```

### Authority and trust boundaries

- `tracekeepd` remains the only SQLite writer.
- Local SQLite remains the runtime source of truth.
- The gateway is a transport queue, not a cloud Tracekeep database.
- The gateway never receives the SQLite file, backup files, or sanitized Git export.
- The PC initiates outbound HTTPS synchronization; `tracekeepd` remains bound to loopback and is never exposed directly to the internet.
...
The mobile path should transfer only what is needed to create or answer a reviewable Tracekeep record:
...
- the user's complete Tracekeep database;
...
The production ChatGPT App requires OAuth 2.1 compatible with the MCP authorization flow. Tracekeep must:

- map one authenticated ChatGPT user to one Tracekeep installation;
...
- **PC offline during recall:** ChatGPT must say that Tracekeep is unavailable or that any cached result may be stale. It must not invent an answer.
...
The local Tracekeep core remains usable without a paid AI API or hosted Tracekeep account. ChatGPT Direct is different: a stable public HTTPS gateway, identity service, monitoring, and data-transfer path may create hosting or operational costs.

Tracekeep must not describe ChatGPT Direct as guaranteed zero-cost. Before Beta, the project must publish who operates the gateway, what is free, what may cost money, and how a user can continue using the local-only Codex and Dashboard path without the gateway.
...
- Prove that an outbound authenticated session can relay online recall without exposing `tracekeepd` or opening an inbound PC port.
...
- Register the Tracekeep tools and concise tool metadata.
...
Until these gates pass, public materials must say **planned ChatGPT Direct mobile support**, not that Tracekeep already works directly inside ChatGPT mobile.
