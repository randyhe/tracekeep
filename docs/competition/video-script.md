...
|---|---|---|
| 0:00–0:14 | Atlas title card | Life interrupts. Valuable learning, half-finished plans, and sudden ideas disappear inside the next conversation. Atlas turns them into a second brain. |
| 0:14–0:42 | Real Codex conversation with synthetic daily-life paper and family-study example | The user talks naturally. No “remember this” command is required. When this meaningful turn ends, the trusted local Atlas hook extracts what is worth keeping. It does not scan every historical chat. |
| 0:42–1:05 | Learning page: conversation note + paper note + visible source | The conclusion and the paper become local Learning Notes automatically, each connected to its source. Atlas can later find what was learned and why it mattered. |
...
| 2:25–2:42 | Local architecture and Windows release card | Atlas runs on loopback with local authentication and SQLite. Codex and GPT-5.6 helped turn the product decisions into tests and a portable Windows release. |
...
- Do not claim automatic access to conversations created before Atlas was
