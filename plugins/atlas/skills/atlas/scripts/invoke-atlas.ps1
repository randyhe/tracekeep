...
if ($BaseUrl -notmatch '^http://(127\.0\.0\.1|localhost)(:\d+)?$') {
    throw "Atlas BaseUrl must use loopback HTTP."
}
...
$ready = Invoke-AtlasRequest -Path "/api/v1/health/ready"
if ($ready.status -ne "ready") {
    throw "Atlas is not ready at $BaseUrl."
}
...
switch ($Operation) {
    "capture" {
        ...
        $result = Invoke-AtlasRequest -Path "/api/v1/captures" -Method "POST" -Body $body
        [pscustomobject]@{
    ...
    "today" { Invoke-AtlasRequest -Path "/api/v1/today" }
    "open-loops" {
        $suffix = if ($Status) { "?status=$([uri]::EscapeDataString($Status))" } else { "" }
        Invoke-AtlasRequest -Path "/api/v1/open-loops$suffix"
    }
    "search" {
        if ([string]::IsNullOrWhiteSpace($Query)) { throw "Query is required for search." }
        Invoke-AtlasRequest -Path "/api/v1/search?q=$([uri]::EscapeDataString($Query))"
    }
    "complete" {
        if ([string]::IsNullOrWhiteSpace($Id) -or $ExpectedVersion -lt 1) { throw "Id and ExpectedVersion are required for complete." }
        Invoke-AtlasRequest -Path "/api/v1/open-loops/$([uri]::EscapeDataString($Id))" -Method "PATCH" -Body @{
            status = "done"
    ...
    "review" { Invoke-AtlasRequest -Path "/api/v1/reviews" }
}
