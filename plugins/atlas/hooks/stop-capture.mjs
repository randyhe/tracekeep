...
async function main() {
  if (String(process.env.ATLAS_AUTO_CAPTURE ?? "on").toLowerCase() === "off") return;
  ...
  const dataDirectory = process.env.PLUGIN_DATA || join(process.env.LOCALAPPDATA || process.cwd(), "Atlas", "plugin-data");
...
async function sendTurn(payload) {
  const baseUrl = normalizeBaseUrl(process.env.ATLAS_BASE_URL || DEFAULT_BASE_URL);
  const response = await fetch(`${baseUrl}/api/v1/imports/codex-turn`, {
    ...
    headers: {
      ...
      ...(process.env.ATLAS_TOKEN ? { authorization: `Bearer ${process.env.ATLAS_TOKEN}` } : {}),
    },
...
function normalizeBaseUrl(value) {
  ...
  if (parsed.protocol !== "http:" || !["127.0.0.1", "localhost"].includes(parsed.hostname)) {
    throw new Error("ATLAS_BASE_URL must use local loopback HTTP");
  }
