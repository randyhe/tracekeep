...
const server = new McpServer({ name: "atlas-memory-local", version: "0.3.0" });
const localReadOnly = {
  readOnlyHint: true,
...
}, localMutation, async (input) => result(await atlas("/api/v1/captures", "POST", {
  ...input,
  sourceType: "codex",
})));

server.tool("get_today", "Get at most three current Atlas focus items.", {}, localReadOnly, async () =>
...
}, localReadOnly, async ({ status }) => result(await atlas(`/api/v1/open-loops${status ? `?status=${status}` : ""}`)));

server.tool("complete_open_loop", "Mark an open loop done using optimistic concurrency.", {
  id: z.string().min(1),
  expectedVersion: z.number().int().positive(),
}, localMutation, async ({ id, expectedVersion }) => result(await atlas(`/api/v1/open-loops/${encodeURIComponent(id)}`, "PATCH", {
...
}, localMutation, async ({ id, expectedVersion, scheduledFor }) => result(await atlas(`/api/v1/open-loops/${encodeURIComponent(id)}`, "PATCH", {
  status: "scheduled", scheduledFor, expectedVersion,
})));

server.tool("search", "Search local Atlas records with SQLite FTS5.", {
  query: z.string().min(1).max(500),
}, localReadOnly, async ({ query }) => result(await atlas(`/api/v1/search?q=${encodeURIComponent(query)}`)));
...
}, localReadOnly, async ({ question }) => result(await atlas(`/api/v1/search?q=${encodeURIComponent(question)}&answer=true`)));
...
server.tool("get_cost_status", "Show Atlas external-cost protections.", {}, localReadOnly, async () =>
